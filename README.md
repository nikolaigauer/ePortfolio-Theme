...
The theme is **fully standalone** — privacy, the process archive, the optional portfolio, the clean ePortfolio templates, and content-type filtering all work on their own. The theme does not provide or depend on any frontend form fields.

For students to *log* activities and reflections via frontend forms, pair it with **Activity Builder** (which owns all submission forms and field config). The two are designed to work together, but neither requires the other for its own features.
